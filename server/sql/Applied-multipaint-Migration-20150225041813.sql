...
--REMOVE: MultiPaint-ChangeBrush-Thickness
Property Thickness will be removed from object ChangeBrush in schema MultiPaint
...
--REMOVE: MultiPaint-Brush-Thickness
Property Thickness will be removed from object Brush in schema MultiPaint
...
ALTER TABLE "MultiPaint"."ChangeBrush" DROP COLUMN IF EXISTS "Thickness";
ALTER TABLE "MultiPaint"."ChangeBrush" DROP COLUMN IF EXISTS "Color";

DO $$ BEGIN
	IF EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = 'Brush' AND column_name = 'Thickness' AND is_ngs_generated) THEN
		ALTER TABLE "MultiPaint"."Brush" DROP COLUMN "Thickness";
	END IF;
END $$ LANGUAGE plpgsql;

DO $$ BEGIN
	IF EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = '-ngs_Brush_type-' AND column_name = 'Thickness' AND is_ngs_generated) THEN
		ALTER TYPE "MultiPaint"."-ngs_Brush_type-" DROP ATTRIBUTE "Thickness";
	END IF;
END $$ LANGUAGE plpgsql;
