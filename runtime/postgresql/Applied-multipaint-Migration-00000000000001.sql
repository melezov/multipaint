...
--CREATE: MultiPaint-RegisterArtist-Name
New property Name will be created for RegisterArtist in MultiPaint
--CREATE: MultiPaint-RegisterArtist-UserID
New property UserID will be created for RegisterArtist in MultiPaint
--CREATE: MultiPaint-RegisterArtist-Password
New property Password will be created for RegisterArtist in MultiPaint
--CREATE: MultiPaint-ChangeArtistName
New object ChangeArtistName will be created in schema MultiPaint
--CREATE: MultiPaint-ChangeArtistName-NewName
New property NewName will be created for ChangeArtistName in MultiPaint
...
DO $$ BEGIN
	IF NOT EXISTS(SELECT * FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'MultiPaint' AND c.relname = 'ChangeArtistName') THEN	
		CREATE TABLE "MultiPaint"."ChangeArtistName" 
		(
			event_id BIGSERIAL PRIMARY KEY,
			queued_at TIMESTAMPTZ NOT NULL DEFAULT(NOW()),
			processed_at TIMESTAMPTZ
		);
		COMMENT ON TABLE "MultiPaint"."ChangeArtistName" IS 'NGS generated';
	END IF;
END $$ LANGUAGE plpgsql;
...
DO $$ 
BEGIN
	IF NOT EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = 'RegisterArtist' AND column_name = 'Name') THEN
		ALTER TABLE "MultiPaint"."RegisterArtist" ADD COLUMN "Name" VARCHAR;
	END IF;
END $$ LANGUAGE plpgsql;

DO $$ 
BEGIN
	IF NOT EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = 'RegisterArtist' AND column_name = 'UserID') THEN
		ALTER TABLE "MultiPaint"."RegisterArtist" ADD COLUMN "UserID" VARCHAR;
	END IF;
END $$ LANGUAGE plpgsql;

DO $$ 
BEGIN
	IF NOT EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = 'RegisterArtist' AND column_name = 'Password') THEN
		ALTER TABLE "MultiPaint"."RegisterArtist" ADD COLUMN "Password" VARCHAR;
	END IF;
END $$ LANGUAGE plpgsql;

DO $$ 
BEGIN
	IF NOT EXISTS(SELECT * FROM "-NGS-".Load_Type_Info() WHERE type_schema = 'MultiPaint' AND type_name = 'ChangeArtistName' AND column_name = 'NewName') THEN
		ALTER TABLE "MultiPaint"."ChangeArtistName" ADD COLUMN "NewName" VARCHAR;
	END IF;
END $$ LANGUAGE plpgsql;
...
SELECT _event.event_id::text AS "URI", _event.event_id, _event.queued_at AS "QueuedAt", _event.processed_at AS "ProcessedAt" , _event."Name", _event."UserID", _event."Password"
FROM
...
BEGIN
	...
	FOR uri IN 
		INSERT INTO "MultiPaint"."RegisterArtist" (queued_at, processed_at, "Name", "UserID", "Password")
...
CREATE OR REPLACE FUNCTION "MultiPaint"."Artist.findActiveUsers"("artist" "MultiPaint"."Artist_entity", "Since" TIMESTAMPTZ) RETURNS BOOL AS 
$$
	SELECT 	 ((("artist"))."LastActiveAt" >= "Artist.findActiveUsers"."Since") 
$$ LANGUAGE SQL IMMUTABLE SECURITY DEFINER;
CREATE OR REPLACE FUNCTION "MultiPaint"."Artist.findActiveUsers"("Since" TIMESTAMPTZ) RETURNS SETOF "MultiPaint"."Artist_entity" AS 
$$SELECT * FROM "MultiPaint"."Artist_entity" "artist"  WHERE 	 ((("artist"))."LastActiveAt" >= "Artist.findActiveUsers"."Since") 
...
DO $$ BEGIN
	IF NOT EXISTS(SELECT * FROM pg_index i JOIN pg_class r ON i.indexrelid = r.oid JOIN pg_namespace n ON n.oid = r.relnamespace WHERE n.nspname = 'MultiPaint' AND r.relname = 'ix_unprocessed_events_MultiPaint_ChangeArtistName') THEN
		CREATE INDEX "ix_unprocessed_events_MultiPaint_ChangeArtistName" ON "MultiPaint"."ChangeArtistName" (event_id) WHERE processed_at IS NULL;
		COMMENT ON INDEX "MultiPaint"."ix_unprocessed_events_MultiPaint_ChangeArtistName" IS 'NGS generated';
	END IF;
END $$ LANGUAGE plpgsql;
...
SELECT "-NGS-".Persist_Concepts('"dsl\\Artist.dsl"=>"module MultiPaint
{
	...
	aggregate Artist(UserID) {
		...
		specification findActiveUsers ''artist => artist.LastActiveAt >= Since'' {
			Timestamp Since;
		}
	}
